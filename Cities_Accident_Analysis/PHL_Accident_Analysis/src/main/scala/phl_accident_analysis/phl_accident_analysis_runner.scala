...
object phl_accident_analysis_runner {
  def main(args: Array[String]):Unit= {

    /*
    val spark = SparkSession.builder() // session builder the gateway to spark-SQL
      .appName("PHL_Accident_Analysis")
      .master("local[4]")
      .getOrCreate()

      val PHL_Files_Location="PHL-FullWeek.json"
    ...
    val PHL_Files_Location="s3://rev-big-data/PHL-FullWeek.json"

    import spark.implicits._ //to enable the use of dollar signs
    spark.sparkContext.setLogLevel("WARN") // avoid unnecessary readouts

    val df = spark.read.option("multiline","true").json(PHL_Files_Location) //read json data
    ...
    val df_General_data = df_un_nest.select(df_un_nest("col.id").alias("Id"),
      df_un_nest("col.entities.hashtags.tag").getItem(0).alias("General_Location"),
      df_un_nest("col.entities.annotations.normalized_text").getItem(0).alias("Incident_Occurrence_Rd"),
      df_un_nest("col.entities.annotations.normalized_text").getItem(1).alias("Detailed_Location"),
    ...
    val Group_Same_General_Location_Name_Distinct_Accident_Table_query="SELECT "+
      "CASE " +
      "WHEN General_Location LIKE 'S' THEN 'SouthPhiladelphia' " +
    ...
    val df_remove_null=spark.sql(General_Road_Location_Query_RH)
        df_remove_null.createTempView("With_null_Rush_hour")

    val General_Road_Location_Query_RH_2="SELECT General_Location, " +
          "Accident_Occurrence_Rd, RUSH_HOUR_COUNT  " +
          "FROM With_null_Rush_hour  " +
          "WHERE Accident_Occurrence_Rd IS NOT NULL " +
          "ORDER BY RUSH_HOUR_COUNT DESC, General_Location, Accident_Occurrence_Rd "+
          ";"
    spark.sql(General_Road_Location_Query_RH_2).show(5,false)

    //Total Accidents for PHILADELPHIA (Timeline: Entire Time)
    val counting_query_ET="SELECT COUNT(*) AS PHILLY_ENTIRE_TIME_ACCIDENT_COUNTS, " +
      "ROUND((COUNT(*)/7),1) AS PHILLY_ENTIRE_TIME_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604880000 AND 1605484740 "+
      ";"

    spark.sql(counting_query_ET).show(5,false)


    //Total Accidents for PHILADELPHIA (Timeline: Weekdays)
    val counting_query_WD="SELECT COUNT(*) AS PHILLY_WEEKDAY_ACCIDENT_COUNTS, " +
      "(COUNT(*)/5) AS PHILLY_WEEKDAY_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604880000 AND 1605311940 "+
      ";"

    spark.sql(counting_query_WD).show(5,false)

    //Total Accidents for PHILADELPHIA (Timeline: Weekends)
    val counting_query_WND="SELECT COUNT(*) AS PHILLY_WEEKENDS_ACCIDENT_COUNTS, " +
      "(COUNT(*)/2) AS PHILLY_WEEKENDS_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1605312000 AND 1605484740 "+
      ";"

    spark.sql(counting_query_WND).show(5,false)

    //Total Accidents for HOUSTON (Timeline: Rush-Hour)
    val counting_query_RH="SELECT COUNT(*) AS PHILLY_RUSH_HOUR_ACCIDENT_COUNTS, " +
      "(COUNT(*)/10) AS PHILLY_RUSH_HOUR_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604905200 AND (1604905200+10800) "+
      "OR Date_Time_Of_Accident BETWEEN (1604905200+32400) AND (1604905200+43200)  " +
      "OR Date_Time_Of_Accident BETWEEN 1604991600 AND (1604991600+10800) " +
    ...
    spark.sql(counting_query_RH).show(5,false)
