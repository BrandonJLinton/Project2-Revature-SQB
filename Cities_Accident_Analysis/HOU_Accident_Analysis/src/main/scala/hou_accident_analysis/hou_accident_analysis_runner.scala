...
object hou_accident_analysis_runner {
  def main(args: Array[String]):Unit= {

    /*
    //Local testing
    val spark = SparkSession.builder() // session builder the gateway to spark-SQL
      .appName("HOU_Accident_Analysis")
      .master("local[4]")
      .getOrCreate()

    val HOU_Files_Location="HOU-FullWeek.json"
    ...
    val HOU_Files_Location="s3://rev-big-data/HOU-FullWeek.json"

    import spark.implicits._ //to enable the use of dollar signs
    spark.sparkContext.setLogLevel("WARN") // avoid unnecessary readouts

    val df = spark.read.option("multiline","true").json(HOU_Files_Location) //read json data
    ...
    val Group_Same_General_Location_Name_Distinct_Accident_Table_query="SELECT "+
      "CASE " +
      "WHEN General_Location LIKE 'NwSide' THEN 'NorthWestSide' " +
      "WHEN General_Location LIKE 'SeSide' THEN 'SouthEastSide' " +
      "WHEN General_Location LIKE 'SwSide' THEN 'SouthWestSide' " +
      "WHEN General_Location LIKE 'NeSide' THEN 'NorthEastSide' " +
    ...
    //Total Accidents for HOUSTON (Timeline: Entire Time)
    val counting_query_ET="SELECT COUNT(*) AS HOUSTON_ENTIRE_TIME_ACCIDENT_COUNTS, " +
      "(COUNT(*)/7) AS HOUSTON_ENTIRE_TIME_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604880000 AND 1605484740 "+
      ";"
    ...
    //Total Accidents for HOUSTON (Timeline: Weekdays)
    val counting_query_WD="SELECT COUNT(*) AS HOUSTON_WEEKDAY_ACCIDENT_COUNTS, " +
      "(COUNT(*)/5) AS HOUSTON_WEEKDAY_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604880000 AND 1605311940 "+
      ";"
    ...
    //Total Accidents for HOUSTON (Timeline: Weekends)
    val counting_query_WND="SELECT COUNT(*) AS HOUSTON_WEEKENDS_ACCIDENT_COUNTS, " +
      "(COUNT(*)/2) AS HOUSTON_WEEKENDS_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1605312000 AND 1605484740 "+
      ";"
    ...
    val counting_query_RH="SELECT COUNT(*) AS HOUSTON_RUSH_HOUR_ACCIDENT_COUNTS, " +
      "(COUNT(*)/10) AS HOUSTON_RUSH_HOUR_ACCIDENT_AVERAGE  "+
      "FROM Distinct_Accidents_Table  " +
      "WHERE Date_Time_Of_Accident BETWEEN 1604905200 AND (1604905200+10800) "+
      "OR Date_Time_Of_Accident BETWEEN (1604905200+32400) AND (1604905200+43200)  " +
      "OR Date_Time_Of_Accident BETWEEN 1604991600 AND (1604991600+10800) " +
