...
of each city over different timelines (Entire Time, Weekdays, Weekends, and Rush hour).Then each city's 
results will be compared. Second part focuses on purely accidents. For each city, the location, 
and the road with highest occurrence of accidents will be shown. Also comparing each city's average 
and amount of accidents against eachother over the same timelines. Lastly an example of real time 
